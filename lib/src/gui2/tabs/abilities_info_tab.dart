    ...
    if (ability != null) {
      try {
        choose = ability.getSafeObject(
            CDOMObjectKey.getConstant<ParsedChoose>('PARSED_CHOOSE')) as ParsedChoose?;
        multOk = ability.getSafeObject(
            CDOMObjectKey.getConstant<bool>('MULT_OK')) as bool? ?? false;
      } catch (_) {}
    }
...
class _SimplePrereqCtx implements PrereqContext {
  ...
  int getStatScore(String statAbb) =>
      statScores[statAbb.toUpperCase()] ?? 10;
