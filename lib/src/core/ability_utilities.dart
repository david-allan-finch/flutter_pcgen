...
class AbilityUtilities {
  ...
  static void finaliseAbility(PlayerCharacter aPC, CNAbilitySelection cnas) {
    ...
    final TransitionChoice<CNAbility>? mc =
        ability.getObject(CDOMObjectKey.modifyChoice) as TransitionChoice<CNAbility>?;
    if (mc != null) {
  ...
  static bool alreadySelected(
    ...
    if (!(ability.getSafeObject(CDOMObjectKey.multipleAllowed) as bool? ?? false)) {
      return true;
    }
    if (allowStack && (ability.getSafeObject(CDOMObjectKey.stacks) as bool? ?? false)) {
      return false;
    }

    final dynamic info = ability.getObject(CDOMObjectKey.chooseInfo);
